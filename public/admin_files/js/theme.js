...
(function($) {
	...
	var LockScreen = {
			        ...
			        success: function (response) {
			            if (response.status == 200) {
			              ...
			              _self.hide();
			                ...
			                $('#lock-error').html('Voer a.u.b. een geldig wachtwoord in.');
			            }
			        },
			        error: function () {
		               $('#lock-error').html('Voer a.u.b. een geldig wachtwoord in.');
		...
		getUserInfo: function() {
			...
			lock    = $info.find( '.profile-info' ).attr( 'data-lock-lock' );
			url    = $info.find( '.profile-info' ).attr( 'data-lock-url' );
			token   = $info.find( '.profile-info' ).attr( 'data-lock-token' );
			return {
				picture: picture,
				username: name,
				email: email,
									...
									'<form action="{{url}}" method="post">',
										...
										'<div class="form-group mb-lg">',
											'<div class="input-group input-group-icon">',
												...
												'<input id="pwd" name="pwd" type="password" class="form-control form-control-lg" placeholder="Wachtwoord" />',
												...
												'</span>',
											'</div>',
											'<span class="help-block alert-danger" id="lock-error"></span>',
										'</div>',

										'<div class="row">',
											'<div class="col-6">',
												'<p class="mt-xs mb-0">',
													'<a href="/signout">Niet {{username}}?</a>',
												'</p>',
											'</div>',
											'<div class="col-6">',
												'<button type="submit" class="btn btn-primary pull-right">Ontgrendel</button>',
											'</div>',
										'</div>',
									'</form>',
								'</div>',
	...
	$(function() {
		...
		if($( '.profile-info' ).attr( 'data-lock-lock')=="1")
		{
			LockScreen.show();
		}
	});
