...
<?php echo app('translator')->getFromJson('user.user_status'); ?>
...
<section role="main" class="content-body">
	...
	<section class="content-header">
	        <ol class="breadcrumb" style="padding: 40px 0 10px 0;font-size: 17px;">
	            <li><a href="/admin/dashboard" style="text-decoration: none;color: #5c5757;"><i class="fa fa-home active"></i>	<?php echo app('translator')->getFromJson('user.user_status'); ?>
				</a></li>
	        </ol>
   	</section>
								...
								<table class="table table-bordered table-striped mb-0" id="datatable-tabletools-status"  data-plugin-options='{"searchPlaceholder": "Zoeken..."}'>
									<thead>
										<tr>
											...
											<th><?php echo app('translator')->getFromJson('common.company'); ?></th>
											<th><?php echo app('translator')->getFromJson('common.status'); ?></th>
											<th><?php echo app('translator')->getFromJson('common.description'); ?></th>
											<th>Status Type</th>
											<th><?php echo app('translator')->getFromJson('common.created_at'); ?></th>
											<th><?php echo app('translator')->getFromJson('common.updated_at'); ?></th>
											<th width="15%"><?php echo app('translator')->getFromJson('common.actions'); ?></th>
										</tr>
									</thead>
					...
					<div class="tabs tabs-dark">
						<ul class="nav nav-tabs">
							...
							<li class="nav-item tab-delete">
								<a class="nav-link" id="delete-status-tab-button"><?php echo app('translator')->getFromJson('common.delete'); ?> </a>
							</li>
						</ul>
						...
						<div class="tab-content">
								...
								<form status="form" id="status_add_form" method="post" enctype="multipart/form-data">
									...
									<div class="">
										<div class="form-group">
											<label for="status"><?php echo app('translator')->getFromJson('user.status_input'); ?></label>
											<input type="text" name="status" class="form-control" id="status" value="<?php echo e(old('status')); ?>" placeholder="<?php echo app('translator')->getFromJson('common.enter_name'); ?>">
											<span class="text-danger" id="status-error"></span>
										</div>
										<div class="form-group">
											<label for="sell_category"><?php echo app('translator')->getFromJson('common.description_input'); ?></label>
											<textarea name="description" class="form-control" id="description" placeholder="<?php echo app('translator')->getFromJson('common.enter_description'); ?>"><?php echo e(old('description')); ?></textarea>
							...
							<div id="update_status" class="tab-pane  tab-edit-pane active">
								<form status="form" id="status_edit_form" method="post" enctype="multipart/form-data">
									...
									<div class="">
										<div class="form-group">
											<label for="days"><?php echo app('translator')->getFromJson('user.status_input'); ?></label>
											<input type="text" name="status" class="form-control" id="edit-status" value="" placeholder="<?php echo app('translator')->getFromJson('common.enter_name'); ?>">
											<span class="text-danger" id="status-error"></span>
										</div>
										<div class="form-group">
											<label for="sell_category"><?php echo app('translator')->getFromJson('common.description_input'); ?></label>
											<textarea name="description" class="form-control" id="edit-description" placeholder="<?php echo app('translator')->getFromJson('common.enter_description'); ?>"></textarea>
											<span class="text-danger" id="description-error"></span>
										</div>
										<div class="form-group">
											<label for="stype">Type</label>
									...
									<div class="modal-footer">
										<button type="button" class="btn btn-default btn-flat pull-left" data-dismiss="modal"><?php echo app('translator')->getFromJson('common.close'); ?></button>
										<button type="button" class="btn btn-info btn-flat update-status-button"><?php echo app('translator')->getFromJson('common.update'); ?></button>
									</div>
								</form>
							</div>
						</div>
...
<div class="modal fade bulk-delete-modal" id="delete-status-modal-bulk" role="dialog">
    <div class="modal-dialog">
      <div class="modal-content">
         <form method="GET" action="<?php echo e(url('admin/bulk-delete/user-status')); ?>" id="delete-status-form-bulk">
        <div class="modal-header">
...
<?php $__env->startSection('site_scripts'); ?> 
	...
	<script>
		...
		function edit(status_id){
			var url = "<?php echo e(route('admin.user_status.show', 'status_id')); ?>";
		...
		$(".update-status-button").click(function(){
			...
			var url = "<?php echo e(route('admin.user_status.update', 'status_id')); ?>";
			url = url.replace("status_id", status_id);
			...
			$.ajax({
				...
				success:function(data) {
					...
					if(data.success) {
						window.location.href = '<?php echo e(route('admin.user_status.index')); ?>';
					}
				},
			});
		});
		$("#store-status-button").click(function(){
			...
			$.ajax({
				type:'POST',
				url:'<?php echo e(route('admin.user_status.store')); ?>',
				processData: false,
				contentType: false,
				data : postData,
				success:function(data) {
		...
		function remove(status_id){
			var url = "<?php echo e(route('admin.user_status.destroy', 'status_id')); ?>";
			url = url.replace("status_id", status_id);
		...
		function view(id){
			var url = "<?php echo e(route('admin.user_status.show', 'id')); ?>";
	...
	<script type="text/javascript">
		...
		var datatableInitStatus = function() {
		    ...
		    bDestroy: true,
			ajax: "<?php echo e(route('admin.getUserStatusRoute')); ?>",
			dom: 'Bfrtip',
			buttons: [
				{
