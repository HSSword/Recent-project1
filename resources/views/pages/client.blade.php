                    ...
                    @foreach($clients->chunk(3) as $chunk)
                     ...
                     @foreach($chunk as $client)
                        <div class="story edit-client-button" >
                            ...
                            @if(Auth::check())
                            @if(Auth::User()->id == 1)
                            <i class="fa fa-edit fa-stack-1x client-edit" data-id="{{ $client->id }}"></i>
                            @endif
                            @endif
                                <div class="image">
                                 @if(empty($client->thumb))
                                    <img src="{{ asset('story_featured_image/'.$client->story_featured_image)}}" alt="">
                                 @else
                                    <img src="{{ asset('story_featured_image/'.$client->thumb)}}" alt="">
                                 @endif
                                </div>
                                <div class="info">

                                    <h3>{{ $client->name }}</h3>
                                    <small>{{ $client->possition }}</small>
                                </div>
                            </div>
                            <div class="text">{!! $client->content !!}</div>
                        </div>
                         @endforeach
                        </div>
                      @endforeach
            ...
            <div class="slider">
                ...
                <div class="images">
                    <div class="image-slide">
                       ...
                       @foreach($gallery_images->chunk(3) as $chunk)
                           <div class="image-row">
	                        @foreach($chunk as $g_image)
	                            <div class="message-image edit-gallery-button"  >
	                            
	                                <p class="message"> 
	                                	@if(Auth::check())
                            			@if(Auth::User()->id == 1)
                                     <i class="fa fa-edit fa-stack-1x client-edit" data-id="{{$g_image->id}}"></i>
                                     @endif
                                     @endif
	                                {{ $g_image->caption }} </p>
	                                @if(empty($g_image->thumb))
	                                <img src="{{ asset('gallery_image/'.$g_image->image)}}" alt="">
	                                @else
	                                <img src="{{ asset('gallery_image/'.$g_image->thumb)}}" alt="">
	                                @endif
	                            </div>
	                        @endforeach
                             </div>
					...
					<div class="modal-header" style="display: block;">
						...
						<form role="form" id="gallery_edit_form" method="post" enctype="multipart/form-data">
							...
							<div class="modal-footer">
								...
								<button type="button" class="btn btn-info btn-flat update-gallery-button">@lang('common.update')</button>
							</div>
						</form>
